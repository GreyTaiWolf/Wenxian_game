...
- 城镇人物：城市和城镇 local map 顶部只显示“城镇人物”入口，不常驻显示放大、缩小、重置、网格和自检按钮；人物、商店和任务通过居中弹窗进入。
...
玩家自由点击并抵达 local map 场景 zone 后，会切换到对应地点内部场景；进入地点仍需要先在大世界 POI 预览卡中确认。local map 的区域、道路、不可通行格、灵气浓度和危险等级由 `src/data/gridMaps.ts` 生成，不新增奖励、任务或战斗组。城市/城镇的 NPC、商店和任务入口统一从“城镇人物”名册或对应地图标记打开居中弹窗，不在地图下方常驻列表卡片。内部地图上的场景/地点标记只显示名称，不显示类型副标题；固定 NPC 不作为地图人物标记显示，而是在点击地点后由地点弹窗的“此地修士”列表承载。点击城镇人物名册或内部地图游历 NPC 标记时，先按该 NPC 的 `homeSceneId`、店铺场景或默认落脚场景寻路；角色抵达后再打开人物对话。点击内部地图地点标记时同样先寻路，抵达后打开“在某地点”的地点弹窗，弹窗列出此地修士和设施；有管事 NPC 的商店/建筑优先由 NPC 打开交易、任务、打造、洗炼等功能，无 NPC 地点才直接显示设施按钮。

### 青云镇内部场景

青云镇进入地点后使用 `local:qingyun_city` 格子 local map。玩家在地图上移动到场景入口后切换当前场景；地图顶部只保留“城镇人物”按钮。地图标记保持轻量，只显示城主府、小小仙铺、赵家炼器铺等地点名称；清雨、赵铁匠、李百草等固定 NPC 不直接铺在地图上，点击地点后在地点弹窗中显示。地点描述、场景列表、资源、事件、敌人、掉落和内联商店不在地图下方常驻展示，后续城镇内容以人物名册、场景动作和事件弹窗承载。
