...
玩家自由点击并抵达中州地点单格入口后，只自动展开地点信息抽屉。青云镇位于中州西侧 `10,12`，与黑风山横向相邻但仍保持单格入口；实际寻路和 zone 命中使用表内 anchor。进入地点后仍使用原有地点场景和行动列表，不新增奖励、任务或战斗组。
...
| 赵家炼器铺 | `zhao_refinery` | 预留 | 交易 | 低阶兵刃与炼器铺，接入现有商店入口 |
...
| 灵田 | `inn_spirit_field` | 预留 | NPC 对话 | 青云客栈老板娘的灵田，当前只展示观察反馈 |
